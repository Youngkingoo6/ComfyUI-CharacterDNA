...
身体 DNA 使用 19 个连续参数描述稳定的身体比例。每项范围为 `-1` 到 `+1`，`0` 是中性基线且不输出提示词。词库默认提供 `-1、-0.5、0、0.5、1` 五档中英文语义，连续数值会使用最接近的词条；最接近 `0` 档时保留 DNA 数值，但省略“均衡”词条。
